...
def print_all(qteor, eps, epsStar, qstar, Ndop=-1):
	...
	print("Оценка погрешности eps* = %.8f"%epsStar)
...
def main():
	...
	pteor = 0.927829							#теор. значение вер. принятия
	qteor = 0.072171								#теор значение вер. отказа
	eps = 0.01									#допустимая погрешность
	ad = 3										#коэф. дов. интервала
	
	probStar = -1								#оценка вероятности
	Ndop = -1									#количество проведений опыта
	...
	while True:
		...
		else:									#уточняем оценку вероятности
			#NA = Na' : N = N'
